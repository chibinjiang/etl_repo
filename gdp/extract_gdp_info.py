...
def main():
    ...
    df = pd.read_csv('./gdp/世界银行gdp_20200908.csv')
    for i, row in df.iterrows():
        ...
        for year in range(1960, 2020):
            if not row.get(str(year)) or pd.isna(row[str(year)]):
                continue
            if CountryGDPModel.get_by(CountryGDPModel.year == year, CountryGDPModel.country_abbr == country_code):
                continue
            model = CountryGDPModel()
            model.country = country_map[country_code]
            model.country_abbr = country_code
            model.year = year
